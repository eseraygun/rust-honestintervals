...
macro_rules! assert_str_eq {
    ($x:expr, $y:expr) => { assert_eq!($x, format!("{:.16}", $y)) };
    ($x:expr, $y:expr, $($arg:tt)+) => { assert_eq!($x, format!("{:.16}", $y), $($arg)+) };
}
...
fn test_from_str_lo() {
    use fp::FromStr;
    use std::f64;
    assert_eq!(0.8999999999999999, f64::from_str_lo("0.9", PREC).unwrap());
    assert_eq!(1.0999999999999999, f64::from_str_lo("1.1", PREC).unwrap());
...
fn test_from_str_hi() {
    ...
    assert_eq!(-0.8999999999999999, f64::from_str_hi("-0.9", PREC).unwrap());
    assert_eq!(-1.0999999999999999, f64::from_str_hi("-1.1", PREC).unwrap());
...
fn test_add_lo() {
    use fp::Add;
    assert_eq!(1.0999999999999999, 0.1.add_lo(1.0));
...
fn test_add_hi() {
    ...
    assert_eq!(-1.0999999999999999, (-0.1).add_hi(-1.0));
}
...
fn test_sub_lo() {
    ...
    assert_eq!(0.8999999999999999, (-0.1).sub_lo(-1.0));
}

#[test]
fn test_sub_hi() {
    use fp::Sub;
    assert_eq!(-0.8999999999999999, 0.1.sub_hi(1.0));
...
fn test_mul_lo() {
    ...
    assert_eq!(-0.9900000000000002, (-1.1).mul_lo(0.9));
}

#[test]
fn test_mul_hi() {
    use fp::Mul;
    assert_eq!(0.9900000000000002, 1.1.mul_hi(0.9));
...
fn test_div_lo() {
    use fp::Div;
    assert_eq!(1.222222222222222, 1.1.div_lo(0.9));
...
fn test_div_hi() {
    ...
    assert_eq!(-1.222222222222222, (-1.1).div_hi(0.9));
}
...
fn test_log_lo() {
    ...
    assert_eq!(-0.6931471805599454, 0.5.log_lo());
...
fn test_log_hi() {
    ...
    assert_eq!(0.6931471805599454, 2.0.log_hi());
}
